  ...
  return <div className="min-h-screen pt-24 pb-16">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-16">
          {drones.map(drone => <div key={drone.id} className="group bg-card rounded-2xl border border-border p-8 hover:border-accent transition-all duration-300 cursor-pointer" onClick={() => setSelectedItem(drone.id)}>
              <div className="aspect-[4/3] relative mb-4 rounded-xl overflow-hidden">
                <img src={drone.src} alt={drone.name} className="w-full h-full object-cover transition-transform duration-300 group-hover:scale-105" />
              ...
              <div className="space-y-2 mb-4">
                {drone.specifications.map((spec, index) => <div key={index} className="flex items-center space-x-2">
